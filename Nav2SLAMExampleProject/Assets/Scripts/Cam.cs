...
public class Cam : MonoBehaviour
{
    ...
    // (lpineda) the step should be computed based on n_channels and 
    // resolutionWidth
    public uint Step = 3*1024;
    ...
    {
        Debug.Log(ShouldPublishMessage);
        if (texture2D != null && camera == this._camera && ShouldPublishMessage)
        {
            Debug.Log("Publishing camera image");
